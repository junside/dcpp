...
<%
	...
	*/
	...
	String param_part2_sym = ComUtil.isNullToDashString(request.getParameter("part_2_sym")); //입사입자표현식
	...
	String orderindex = request.getParameter("gotoindex"); //선택되어진 항목값
	String _oCol = ComUtil.isNullToDashString(request.getParameter("_oCol")); //Order 대상
	String _oDir = ComUtil.isNullToDashString(request.getParameter("_oDir")); //Ordrer 순서
	...
	String str_where_option_order_by = "";
	String str_where_option_data_and = " AND BASIC.PL_BI_DATA_NUM = GRAPH.PL_BI_DATA_NUM ";	
	
	//System.out.println("_oDir : " + _oDir);
	//System.out.println("_oCol : " + _oCol);
	
	if("-".equalsIgnoreCase(_oDir)){
		_oDir = "asc";
	}	
	if("desc".equalsIgnoreCase(_oDir)) {
		_oDir = "desc";
	}
	if("pr_no".equals(_oCol)) { //물성번호
		str_where_option_order_by = " ORDER BY BASIC.PL_BI_DATA_NUM " + _oDir;				
	}
	else if("imp_met".equals(_oCol)) //충돌방식
	{
		str_where_option_order_by = " ORDER BY BASIC.PL_BI_IMP_CLASS " + _oDir;	
	}
	else if("sub_pro".equals(_oCol)) //부프로세스
	{
		str_where_option_order_by = " ORDER BY BASIC.PL_BI_SUB_PROC " + _oDir;	
	}
	else if("exp_the".equals(_oCol)) //이론/실험/추천
	{
		str_where_option_order_by = " ORDER BY BASIC.PL_BI_EXP_THE_REC " + _oDir;	
	}
	else{
		str_where_option_order_by = " ORDER BY BASIC.PL_BI_DATA_NUM, BASIC.PL_BI_IMP_CLASS ASC ";	
	}
	//조건절 조건 만들기
	//String str_where_option_pr_no=" ORDER BY BASIC.PL_BI_DATA_NUM";
	//String str_where_option_impact=" ORDER BY BASIC.PL_BI_IMP_CLASS";
	//String str_where_option_subpro=" ORDER BY BASIC.PL_BI_SUB_PROC";
	//String str_where_option_recthe=" ORDER BY BASIC.PL_BI_EXP_THE_REC";
	
	//String str_where_option_od_asc = " ASC "; //내림
	//String str_where_option_od_desc = " DESC "; //오름
...
function viewGraphInfo(pr_no){ //그래프 보기
	if(saveMsg('0') == true){
		window.open("pr_view_graph.jsp?pr_no="+pr_no,"graph_info","width=100%,height=100%,toolbar=no,location=no,status=no");
		return;
	}
}
...
function viewBasicInfo(pr_no, artcl_no){ //기본정보 보기
	if(saveMsg('3') == true){
		window.open("pr_view_property_info.jsp?pr_no="+pr_no+"&artcl_no="+artcl_no,"property_info","width=100%,height=100%,toolbar=no,location=no,status=no");
		return;
	}
}
...
function saveMsg(number){
	...
	switch(num) {
		...
		case '3': //기본정보 보기
			...
			break;
		case '4': //Text 정보 보기
			var result = confirm("물성 정보를 Text 형태로 확인하시겠습니까?");
			return result;
			break;
		default :		
			break;
...
function getPropertyInfo(){
	...
	if(count == 0){		
		alert("확인하려는 물성 번호의 체크박스를 선택하세요.");
		return;
	}else{
		if(check[0].checked==true && count==1){
			...
			if(saveMsg('4') == true){
				//alert("체크박스 선택 :" + count);
				//alert(check_value);
				window.open("pr_view_text_property_info.jsp?check="+check_value,"property_info","width=700,height=600,scrollbars=yes");
				return;
			}
		}
...
<p class="level2">검색 조건</p>
<span class="help"><font color = red>대분류 : </font><%=param_TB_exp%></span><br/>
<span class="help"><font color = red>데이터분류 : </font><%=param_DB_exp%></span><br/>
<span class="help"><font color = red>주프로세스 : </font><%=param_MP_exp%></span><br/>
<span class="help"><font color = red>표적입자 : </font><%=param_part1_sym%></span><br/>
<p class="level2">그래프</p>
<table class="tab5">
<col width="110">
<col width="540">
  <tr>
...
<p class="level2">물성 데이터 리스트</p>
<span class="help">
수치데이터를 확인하시려면 그래프를 클릭하세요.
</span><br>
<span class="help">
참고문헌을 확인하시려면 물성번호를 클릭하세요.
</span><br>
<span class="count">
데이터 : <%=totalRecord%>건 
</span>
...
<tr>
   ...
   <th class="thc">
    	 <A HREF="javascript:setOrderDirection('pr_no')">물성번호
    	 <%
    	 	//내림차순, 오름차순 이미지 표시
    	 	if("pr_no".equalsIgnoreCase(_oCol) || "-".equalsIgnoreCase(_oCol)){
    	 		if("-".equalsIgnoreCase(_oDir) || "asc".equalsIgnoreCase(_oDir)){
    	 			%>
    	 				<img src = "images/img_cont/tri_asc.gif" width="10" height="7" border="0" title="asc" alt="asc">
    	 			<%
    	 		}else{
    	 			%>
	 				<img src = "images/img_cont/tri_desc.gif" width="10" height="7" border="0" title="desc" alt="desc">
	 				<%
    	 		} 
    		}    	 		
    	 %>
   ...
   <th class="thc">
     	반응식
   		...
   		<A HREF="javascript:setOrderDirection('imp_met')">충돌방식
   		<%
    	 	//내림차순, 오름차순 이미지 표시
    	 	if("imp_met".equalsIgnoreCase(_oCol)){
    	 		if("-".equalsIgnoreCase(_oDir) || "asc".equalsIgnoreCase(_oDir)){
    	 			%>
    	 				<img src = "images/img_cont/tri_asc.gif" width="10" height="7" border="0" title="asc" alt="asc">
    	 			<%
    	 ...
    	 %>
   		...
   		<A HREF="javascript:setOrderDirection('sub_pro')">부프로세스
   		<%
    	 	//내림차순, 오름차순 이미지 표시
    	 	if("sub_pro".equalsIgnoreCase(_oCol)){
    	 		if("-".equalsIgnoreCase(_oDir) || "asc".equalsIgnoreCase(_oDir)){
    	 			%>
    	 				<img src = "images/img_cont/tri_asc.gif" width="10" height="7" border="0" title="asc" alt="asc">
    	 			<%
   ...
   <th class="thc">
     	<A HREF="javascript:setOrderDirection('exp_the')">이론/실험/추천
     	<%
    	 	//내림차순, 오름차순 이미지 표시
    	 	if("exp_the".equalsIgnoreCase(_oCol)){
    	 		if("-".equalsIgnoreCase(_oDir) || "asc".equalsIgnoreCase(_oDir)){
    	 			%>
    	 				<img src = "images/img_cont/tri_asc.gif" width="10" height="7" border="0" title="asc" alt="asc">
    	 			<%
...
<div class="bbtn_r">
<a href="javascript:getPropertyInfo()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('getListImage','','images/img_cont/get_info_list_upper.gif',1)"><img src="images/img_cont/get_info_list_normal.gif" name="getListImage" width="150" height="24" border="0" id="getListImage" /></a>
</div>
</form>
</body>
</html>