...
<%
	...
	*/
	...
	//기본 정보 가져오기
	Inf_Grd_Graph_Basic_Info graph = (Inf_Grd_Graph_Basic_Info)graph_info.selectViewInfGrdGraphInfo(v_pr_no);
...
function viewDataInfo(v_pr_no){ //수치정보 보기
	window.open("pr_view_inf_grd_graph_data.jsp?v_pr_no="+v_pr_no,"property_info","width=700,height=650,toolbar=no,location=no,status=no");
	return;
}
...
<div id="content">
<p class="level2">등급유력 물성 데이터 및 그래프 표현</p>
<table class="tab2">
<col width="110">
<col width="540">
  <tr>
    <th class="thc">등급유력번호</th>
    <td class="tdl">
	    <%
	    	if(ComUtil.isNull(v_pr_no)){
	    %>
	    	자동으로 선택됩니다. 
	    <%
	    	}else{
  ...
  <tr>
	<th class="thc">수치데이터</th>   
  	<td class="tdl"> 
  	  <a href="javascript:viewDataInfo('<%=v_pr_no%>')" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('viewImage','','images/img_cont/view_s_upper.gif',1)"><img src="images/img_cont/view_s_normal.gif" name="viewImage" width="60" height="20" border="0" id="viewImage" /></a>  	</td>
  <tr>
