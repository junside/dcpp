...
<%
/**
* 참고문헌 데이터를 입력하는 폼 JSP 페이지(일반 참고문헌 입력에서 호출시)
* 저장과 동시에 선택
* by Junside(J.H Park)
*/
...
function uploadArtclInfo(){ //업로드
	...
	//2. 논문 저자 성 체크
	if(form2.pl_ra_artcl_auth_fname.value.length < 1){
		alert("논문 저자 성(Family Name)을 입력하세요!");
		form2.pl_ra_artcl_auth_fname.focus();
		errorV = 'T';
		return;
	}
	
	//3. 논문 저자 이름 체크
	if(form2.pl_ra_artcl_auth_ename.value.length < 1){
		alert("논문 저자 이름을 입력하세요!");
		form2.pl_ra_artcl_auth_ename.focus();
		errorV = 'T';
		return;
	}
	...
	if(errorV == 'F'){
		form2.action = 'pr_article_insert_search_fileupload.jsp';
		form2.submit();
	}
}
...
<span class="help">논문 제목 입력시 주의할 점은 -> 나 <- 를 표시할 때는 특수문자로 입력하면 안됩니다.</span><br>
<table class="tab2">
	<col width="20%">
	<col width="80%">
	<tr>
	...
	<tr>
		<th class="thla">논문 저자 성(Family Name)</th>
		<td class="tdl"><input type='text' class='txtl300' name='pl_ra_artcl_auth_fname' size='55' value=''></td>
	</tr>
	<tr>
		<th class="thla">논문 저자 이름 </th>
		<td class="tdl"><input type='text' class='txtl300' name='pl_ra_artcl_auth_ename' size='55' value=''></td>
	</tr>
	<tr>
	 ...
	 <tr>
		...
		<td class="tdl"><input type='text' class='txtl300' name='pl_ra_artcl_doi' size='55' value=''></td>
	</tr>
    <tr>
