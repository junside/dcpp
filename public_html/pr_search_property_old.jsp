	...
	function selectPartID(part_id){
		...
		if(searchInfoCheck("1") == false){
			if(part_id == 'part_1_sym'){
				...
				window.open("pr_select_target_part.jsp?id="+id+"&sym="+sym+"&db="+db+"&mp="+mp,"select_t_part","width=700,height=600,scrollbars=yes");
			}else{
				var id = "part_2_id";//id
				var sym = "part_2_sym";//sym	
				window.open("pr_select_projectile_part.jsp?id="+id+"&sym="+sym+"&db="+db+"&mp="+mp+"&t_id="+t_id,"select_p_part","width=700,height=600,scrollbars=yes");
			}
		}
